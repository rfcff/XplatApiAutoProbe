...
|---|---|---|---|
| ver=2 RTC API（createEngine / joinRoom / subscribe / preview / getState …） | ✅ Thunder | ✅ 模拟器；`XPROBE_USE_THUNDER=1` 接真实 Thunder | ✅ 模拟器；Windows `-DXPROBE_USE_THUNDER=ON` |
| echo / scheduleCallback 协议探针 | ✅ | ✅ | ✅ |
| ver=1 反射 + field | ✅ | ✅ DemoCalc / DemoState；Thunder 模式下含 Engine/Canvas | ❌ C++ 无反射（透传 legacyApi） |
| 手动 UI | Material | AppKit | Dear ImGui + GLFW |
| Headless / CI | adb 起 App | `--headless`（模拟器） | `--headless`（模拟器） |
...
- PC：本仓库 `demo/pc` 已支持可选真实 Thunder（Windows `LoadLibrary`，见 [`pc/README.md`](pc/README.md)）；完整 MFC 面参考 `ThunderBoltDemo-pc_3.12.0_mb`

本仓库 demo 默认用 **RtcManager 模拟器**（Android 除外，始终真 Thunder）覆盖自动化协议回归，不强制拉取私有 Thunder 包。
...
| [pc/](pc) | `-DXPROBE_USE_THUNDER=ON`（仅 Windows） | 运行时 `LoadLibrary`（`XPROBE_THUNDER_DLL`），见 [pc/README.md](pc/README.md) |

完整 UI / API 面仍以 ThunderBoltDemo 参考仓为准；本仓库只接 Android 对齐的最小 RTC 面。

| 目录 | 平台 | 构建方式 | 产物 / 入口 |
|---|---|---|---|
| [pc/](pc) | Windows / macOS / Linux | CMake（可选 ImGui UI；可选 Thunder） | `pc/build/xprobe_demo [--headless]` |
...
./demo/pc/build/xprobe_demo --headless   # CI
./demo/pc/build/xprobe_demo              # ImGui UI
