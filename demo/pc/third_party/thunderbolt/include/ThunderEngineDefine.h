...
enum ThunderRet
{
  ...
  THUNDER_RET_ALREADY_START_VIDEO_PUBLISH = -28, // 已启动视频开播，设置用大小流模式开播必须在开播之前
  THUNDER_RET_START_AUDIO_ENCODE_ERR = -29, // 启动音频编码出错
  THUNDER_RET_ALREADY_START_VIDEO_CAPTURE = -30, // Video capture has started
  THUNDER_RET_LOCAL_USER_ROLE_ERR = -31,// 用户角色错误，只有用户角色为主播才能发布音视频流
  THUNDER_RET_NOT_SUPPORT_IN_LOWLANTENCY = -32, // 不支持在低延迟THUNDER_RET_NOT_SUPPORT_IN_LOWLANTENCY模式下调用
  THUNDER_RET_ALREADY_START_LAST_MILE_PROBE_TEST = -33, // The test of lastmile is running
  THUNDER_RET_ALREADY_STOP_LAST_MILE_PROBE_TEST = -34, // The test of lastmile is closed
  THUNDER_RET_UP_OR_DOWN_SHOULD_BE_TRUE = -35, // One probe must be specified for uplink and downlink
  THUNDER_RET_BITRATE_RANGE_ERROR = -36, // Bit rate range error
  ...
  // 订阅相关 【-201 ~ -300】
  THUNDER_RET_ALREADY_SUBSCRIBE_ALL_ROOM = -201, // Users who have subscribed to the whole room cannot unsubscribe or subscribe to a single user by using the addsubscribe and removesubscribe interfaces
  THUNDER_RET_NO_SUBSCRIBE_ROOM = -202, // There is no subscription to this room
  THUNDER_RET_NO_SUBSCRIBE_UID = -203, // This user is not in the subscription room
