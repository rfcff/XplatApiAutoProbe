...
class AutomatorServer(object):
    ...
    def install(self):
        base_dir = os.path.dirname(__file__)
        print("%-20s" % self.adb.device_serial(), "===============install=====================")

        # add by barton
        def waitForDevice():
            print("%-20s" % self.adb.device_serial(), "AutomatorServer wait for device")
            self.adb.cmd("wait-for-device").wait()
            # self.adb.cmd("devices").wait()          
            while (u"package:com" not in self.adb.cmd("shell", "pm", "list", "packages").communicate()[0].decode(
                    "utf-8").strip()):
                # print "==="
                # print self.adb.cmd("shell", "pm", "list", "packages").communicate()[0].decode("utf-8").strip()
                # print "==="
                time.sleep(1)
            print("%-20s" % self.adb.device_serial(), "AutomatorServer device connected")

            # 等待手机连接成功才继续，因为中途手机可能被拔掉

        waitForDevice()
        output = self.adb.cmd("shell", "pm", "list", "packages", "uiautomator").communicate()[0].decode("utf-8").strip()
        if "com.github.uiautomator.test" in output:
            time.sleep(5)
            return
        ...
        def _JsonRPCMethod(url, method, timeout, restart=True):
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    print("%-20s" % self.adb.device_serial(), e, "url:", url, "method:", method)
                    if self.bStop:
                        print("%-20s" % self.adb.device_serial(), "stop error,end !!!")
                        raise  # stop 服务时 方法异常直接抛出错误不再重启

                    if restart:
                        print("%-20s" % self.adb.device_serial(), "=============restart============= (URLError, socket.error, HTTPException) ")
                        self.restart = True
                    ...
                    if e.code >= ERROR_CODE_BASE - 1:
                        print("%-20s" % self.adb.device_serial(), "=============restart============= e.code >= ERROR_CODE_BASE - 1          ")
                        self.restart = True
                        server.stopForRestart()
                        server.start()
                        return _method_obj(*args, **kwargs)
    ...
    def start(self, timeout=5):
        if self.sdk_version() < 18:
            ...
            self.install()
            cmd_stop = ["shell", "am", "force-stop", "com.github.uiautomator"]
            self.adb.cmd(*cmd_stop).wait()
        ...
        print("%-20s" % self.adb.device_serial(), "AutomatorServer start end")
    ...
    def stopForRestart(self):
        '''Stop the rpc server.'''
        if self.bStop:
            return
        print("%-20s" % self.adb.device_serial(), 'Stop the rpc server For restart.')
        if self.uiautomator_process and self.uiautomator_process.poll() is None:
            ...
            try:
                ...
                print("%-20s" % self.adb.device_serial(), "UiAutomator __init__ stop Exception:", e)
    ...
    def stop(self):
        '''Stop the rpc server.'''
        self.__bStop = True
        print("%-20s" % self.adb.device_serial(), 'Proactive Stop the rpc server.')
        if self.uiautomator_process and self.uiautomator_process.poll() is None:
            res = None
            try:
                res = urllib2.urlopen(self.stop_uri)
    ...
    @property
    def stop_uri(self):
        stop_url = "http://%s:%d/stop" % (self.adb.adb_server_host, self.local_port)
        print("%-20s" % self.adb.device_serial(), "stop_url:", stop_url)
        return stop_url
